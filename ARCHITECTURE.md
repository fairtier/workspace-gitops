...
| `duckflight` (2)          | OCI chart `ghcr.io/fairtier/charts/duckflight` **0.0.5** (image `0.1.0`; 0.0.5 = tag `helm/v0.0.5`, restricted-PSA securityContexts + no SA token by default) + [duckflight/values.yaml](./charts/duckflight/values.yaml) + [duckflight/ingress/](./charts/duckflight/ingress/)                                                                                                                                                                                                                                                                                                                                                     | `fairtier-system` |
...
| `rill` (2)                | [rill/](./charts/rill/) (mini chart, image `rilldata/rill:v0.87.2` + oauth2-proxy `v7.15.3` + snapshot-sidecar `0.3.0` in git mode + `rill-deploy-shim` `0.0.1`, enabled — it fronts Rill and turns the Deploy button into a `TriggerSnapshot`) — the box BI tool; **two instances of the same project**: the editor (`rill.`) and a read-only `--preview` viewer (`dashboards.`), each with its own PVC + oauth2-proxy; project content synced with the on-box Gitea repo (see [Rill project git sync](#rill-project-git-sync))                                             | `fairtier-system` |
