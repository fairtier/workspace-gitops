...
| `cert-manager` (0)        | chart `jetstack/cert-manager` **v1.20.2** + [cert-manager/values.yaml](./charts/cert-manager/values.yaml) + [cert-manager/issuers/](./charts/cert-manager/issuers/) (mini chart: `letsencrypt-prod` ClusterIssuer)                                                                                                                                                                                                                                                                                                                              | `cert-manager`    |
...
| `openfga` (1)             | [openfga/](./charts/openfga/) (kustomize, image `openfga/openfga:v1.17.1`)                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `fairtier-system` |
...
| `rill` (2)                | [rill/](./charts/rill/) (mini chart, image `rilldata/rill:v0.87.2` + oauth2-proxy `v7.15.4` + snapshot-sidecar `0.3.2` in git mode + `rill-deploy-shim` `0.0.3`, enabled — it fronts Rill and turns the Deploy button into a `TriggerSnapshot`) — the box BI tool; **two instances of the same project**: the editor (`rill.`) and a read-only `--preview` viewer (`dashboards.`), each with its own PVC + oauth2-proxy; project content synced with the on-box Gitea repo (see [Rill project git sync](#rill-project-git-sync))                                             | `fairtier-system` |
