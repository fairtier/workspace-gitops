# The workspace machine, component by component
...
> ([box-release/](./charts/box-release/)), gated **off** by `releaseAgentEnabled`
...
1. a `HelmChart` CR installing **Argo CD** via the k3s built-in
   ...
   install only**: after the first sync the [argocd/](./charts/argocd/) app adopts
   this CR, so day-2 upgrades come from the `argocdChartVersion` pin in
   [charts/root/values.yaml](./charts/root/values.yaml);
...
3. the root app-of-apps `Application` — the canonical copy is
   ...
   **self-managed** ([charts/root/templates/root-app.yaml](./charts/root/templates/root-app.yaml)),
...
([cloud-init drift](#cloud-init-drift)). [bootstrap-retire/](./charts/bootstrap-retire/)
...
[root-app.yaml](./root-app.yaml) points at [apps/](./charts/root/), a small Helm
...
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------|
| `argocd` (0)              | [argocd/](./charts/argocd/) (mini chart adopting the bootstrap `HelmChart` CR — day-2 ArgoCD upgrades; the release's **values** live in a [`HelmChartConfig`](./charts/argocd/templates/helmchartconfig.yaml) so they outrank the frozen cloud-init copy, see [cloud-init drift](#cloud-init-drift))                                                                                                                                                                                                                                                                                                                                                                                                                                   | `kube-system`     |
| `box-root` (0)            | [charts/root/templates/root-app.yaml](./charts/root/templates/root-app.yaml) (self-managed root app, `Prune=false`)                                                                                                                                                                                                                                                                                                                                                                                                                             | `argocd`          |
| `cert-manager` (0)        | chart `jetstack/cert-manager` **v1.20.4** + [cert-manager/values.yaml](./charts/cert-manager/values.yaml) + [cert-manager/issuers/](./charts/cert-manager/issuers/) (mini chart: `letsencrypt-prod` ClusterIssuer)                                                                                                                                                                                                                                                                                                                              | `cert-manager`    |
| `postgres` (0)            | [postgres/](./charts/postgres/) (kustomize, `postgres:17-alpine` StatefulSet + `postgres_exporter` v0.20.1 metrics sidecar on `:9187`, see [PostgreSQL metrics](#postgresql-metrics))                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `fairtier-system` |
| `casdoor` (1)             | [casdoor/](./charts/casdoor/) (mini chart, image `casbin/casdoor:3.153.0`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `fairtier-system` |
| `workspace-db` (1)        | [workspace-db/](./charts/workspace-db/) (plain manifests, `migrate/migrate` Sync-hook Job) — schema of the box-local `workspace` database (created by the postgres ensure-databases Job): local-first run history for the dlt-worker on a box that has **not** been cut over, the box half of the control-plane/workspace-plane state inversion; migrations are additive-only and column-compatible with the central run tables. **Dead once `workspacePlaneLocal` is on** (Phase 3D): the worker then records runs straight into `workspace_api`, where the Console reads them, so this database stops being written and is kept only as the rollback target                                        | `fairtier-system` |
| `openfga` (1)             | [openfga/](./charts/openfga/) (kustomize, image `openfga/openfga:v1.20.0`)                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `fairtier-system` |
| `lakekeeper` (2)          | chart `lakekeeper` **0.11.0** (image `v0.12.4`) + [lakekeeper/values.yaml](./charts/lakekeeper/values.yaml) + [lakekeeper/ingress/](./charts/lakekeeper/ingress/)                                                                                                                                                                                                                                                                                                                                                                               | `fairtier-system` |
| `duckflight` (2)          | OCI chart `ghcr.io/fairtier/charts/duckflight` **0.0.7** (image `0.2.2`; 0.0.7 = tag `helm/v0.0.7`, where `extensions.image` means EXTRAS — a union over the set baked into the server image, unset = no initContainer; 0.0.6 added the reconcileSql mount + tempDir; 0.0.5 brought restricted-PSA securityContexts + no SA token by default) + [duckflight/values.yaml](./charts/duckflight/values.yaml) + [duckflight/ingress/](./charts/duckflight/ingress/)                                                                                                                                                                                                                                                                                                                                                     | `fairtier-system` |
| `gitea` (2)               | [gitea/](./charts/gitea/) (mini chart, image `docker.gitea.com/gitea:1.27.3-rootless`) — hosted git for transformation repos                                                                                                                                                                                                                                                                                                                                                                                                             | `fairtier-system` |
| `rill` (2)                | [rill/](./charts/rill/) (mini chart, image `rilldata/rill:v0.87.8` + oauth2-proxy `v7.15.4` + snapshot-sidecar `0.3.2` in git mode + `rill-deploy-shim` `0.0.3`, enabled — it fronts Rill and turns the Deploy button into a `TriggerSnapshot`) — the box BI tool; **two instances of the same project**: the editor (`rill.`) and a read-only `--preview` viewer (`dashboards.`), each with its own PVC + oauth2-proxy; project content synced with the on-box Gitea repo (see [Rill project git sync](#rill-project-git-sync))                                             | `fairtier-system` |
| `dlt` (2)                 | [dlt/](./charts/dlt/) (mini chart, image `ghcr.io/fairtier/dlt-worker:0.17.0` + snapshot-sidecar `0.3.2` in git mode) — ingestion worker; schedules from the box `pipelines` checkout and decrypts `*.credentials.age` from it with the box age identity, writes Iceberg via the on-box Lakekeeper, state synced with the on-box Gitea repo (see [dlt state git sync](#dlt-state-git-sync)). Since the Phase 3C cutover `FAIRTIER_API_URL` points at the box's own `workspace-api:8081` — there is no phone-home, and since split Phase 4 nowhere else it could point. That poll is a **trigger feed**, not a config feed: Phase 2.5 shrank `GetPipelineConfigs` to Run-now triggers, `file_upload` storage credentials and the last-run watermark, and worker 0.9.0 requires the checkout (`PIPELINES_DIR`) | `fairtier-system` |
| `iceberg-maintenance` (2) | [iceberg-maintenance/](./charts/iceberg-maintenance/) (mini chart, image `ghcr.io/fairtier/iceberg-maintenance:0.10.0`, which bakes pyiceberg `0.12.0`) — nightly CronJob: small-file compaction, snapshot expiry (7-day time-travel window) and an orphan-file sweep (Lakekeeper's maintenance task queues are enterprise-only, so the sweep is ours). The sweep runs in `dry-run` on every box whose `orphanSweepMaxDeletes` is 0 — a per-box helm parameter on that box's own `box-root` Application, and [charts/root/values.yaml](./charts/root/values.yaml) records why it is deliberately not a slug-keyed value in this repo — so arming is **per box** and needs that box's own live-set proof first, which the `iceberg_maintenance.verify` module produces; auths as the `dlt-oidc` client                                                                                                       | `fairtier-system` |
| `box-secrets` (2)         | [box-secrets/](./charts/box-secrets/) (mini chart, `alpine/k8s` Sync-hook Job + 15-minute CronJob) — the central→box credential channel: mints a box Casdoor token, calls `BoxCredentialService.FetchBoxSecrets`, and reconciles the answer into Kubernetes Secrets (into other namespaces too, where the consumer lives — the mapping in [box-secrets/values.yaml](./charts/box-secrets/values.yaml) names one). **Wave 2 is a floor, not a preference**: it authenticates with the `dlt-oidc` client that casdoor seeds at wave 1, so nothing earlier can work and no consumer before wave 3 can read a box secret. A failed or 501 fetch leaves existing Secrets untouched — a central rollback must never blank a live credential | `fairtier-system` |
| `box-release` (2)         | [box-release/](./charts/box-release/) (mini chart, `alpine/k8s` Sync-hook Job + CronJob) — **the deploy agent, and the whole of it**: mints a box Casdoor token, asks the control plane `release.v1.ReleaseService/GetBoxRevision`, and patches `box-root`'s `targetRevision` and its mirroring helm parameter when the answer differs. RBAC is `get`+`patch` on **one Application by name**, nothing else, and there is no `list`. A non-commit answer is refused rather than applied; an unreachable or silent control plane leaves the pin untouched. Gated off by `releaseAgentEnabled`. Deleting this Application removes every path by which anyone but the box's own operator can deploy to it — nothing else depends on it | `fairtier-system` |
| `workspace-api` (3)       | [workspace-api/](./charts/workspace-api/) (mini chart, image `ghcr.io/fairtier/workspace-api:0.37.2` — the public [workspace plane](https://github.com/fairtier/workspace-api) the central FairTier API also imports as a Go module) — the product RPCs served **from the box**: box Postgres (`workspace_api` database, migrated by the binary) + box Gitea as source of truth, box Casdoor as the only trusted issuer, adopt + stuck-run sweeps in-process. The database starts empty and the adopt sweep **hydrates it from the box repos** (0.4.0+, `importFromRepo` lever): untracked `pipelines/*.yaml` / `transformations/*.yaml` are imported as rows keeping the ids the files carry, read-only toward the repos. Public mux published at `api.customer-<slug>.<baseDomain>`; the internal `:8081` (worker poll) is Service-only. **0.6.0 (Phase 3C)** is what lets a browser reach it: `GET /.well-known/fairtier-workspace` (unauthenticated — the Console cannot present a token before it learns which Casdoor to ask) advertises the box's issuer, org and `console` client id, and the seed Job mints that Casdoor app + Secret `console-oidc`; `AUTH_EXPECTED_AUDIENCES` then binds sessions to it so tokens minted for `rill`/`duckflight`/`dlt-worker` cannot reach the product RPCs (lever `enforceConsoleAudience`). Ships **shadow** until the per-customer cutover marker is set (split Phase 3C/3D); fleet lever `workspaceApiEnabled` in [charts/root/values.yaml](./charts/root/values.yaml)                                                                                            | `fairtier-system` |
| `backup` (3)              | [backup/](./charts/backup/) (mini chart, `postgres:17-alpine` + `ghcr.io/rclone/rclone:1.75.1`) — nightly CronJob: pg_dump of every box database + tar of the Gitea PVCs → the customer's own bucket under `backups/`, 7-day retention (see [Backups](#backups))                                                                                                                                                                                                                                                                         | `fairtier-system` |
| `observability` (3)       | chart `grafana/alloy` **1.12.1** + [observability/values.yaml](./charts/observability/values.yaml) + [observability/config/](./charts/observability/config/) (mini chart: per-box River config + kubelet-metrics RBAC) — ships pod logs → central Loki (tenant = slug) and node/ArgoCD/cert-manager/cadvisor metrics → central Prometheus, outbound-only through `ingest.<baseDomain>` (see [Fleet telemetry](#fleet-telemetry-alloy--central)); local OTLP `:4317` receiver keeps DuckFlight's exporter target resolvable (traces dropped, v1). Behind `betterstack.enabled` (on since 2026-08-15) the same three signals also go **direct** to this box's own Better Stack source — a second remote_write endpoint, an `otelcol.receiver.loki` bridge for logs, and a real destination for traces instead of the blackhole; credentials arrive through `box-secrets`, never cloud-init. Additive only: the central push is untouched either way | `observability`   |
| `system-upgrade` (3)      | [system-upgrade/](./charts/system-upgrade/) (vendored rancher/system-upgrade-controller **v0.20.1** manifests + the `k3s-server` Plan) — k3s day-2 upgrades: the Plan pins the fleet to `k3sVersion` in [charts/root/values.yaml](./charts/root/values.yaml); bumping the pin upgrades k3s through the normal GitOps rollout (cordon-only on the single node, no drain)                                                                                                                                                                                  | `system-upgrade`  |
...
Where they live: [argocd](./charts/argocd/templates/helmchartconfig.yaml) (a
`HelmChartConfig`, so the frozen cloud-init copy of the `HelmChart` cannot
revert them — [cloud-init drift](#cloud-init-drift)),
[traefik](./charts/traefik/) (also a `HelmChartConfig`,
since k3s owns that release), and each app's own `values.yaml` /
`deployment.yaml`. Note [lakekeeper](./charts/lakekeeper/values.yaml): its block
...
[`letsencrypt-prod`](./charts/cert-manager/issuers/templates/clusterissuer-letsencrypt-prod.yaml),
...
> [box-secrets/](./charts/box-secrets/) instead, because a value delivered here is
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| Secret `postgres-credentials`                                                                                                                                            | `postgres-password` (superuser)                                                                                                                                                                                                                      | [postgres/statefulset.yaml](./charts/postgres/statefulset.yaml)                                       |
| Secret `casdoor-conf`                                                                                                                                                    | `dataSourceName` (full beego DSN incl. password, `dbname=casdoor`)                                                                                                                                                                                   | [casdoor deployment](./charts/casdoor/templates/deployment.yaml) (env override of `app.conf`)         |
| Secret `openfga-postgres`                                                                                                                                                | `datastore-uri` (postgres URI incl. password, db `openfga`)                                                                                                                                                                                          | [openfga](./charts/openfga/)                                                                          |
| Secret `lakekeeper-postgres`                                                                                                                                             | `postgres-user`, `postgres-password`                                                                                                                                                                                                                 | [lakekeeper/values.yaml](./charts/lakekeeper/values.yaml)                                             |
| Secret `lakekeeper-secrets`                                                                                                                                              | `encryption-key`                                                                                                                                                                                                                                     | [lakekeeper/values.yaml](./charts/lakekeeper/values.yaml)                                             |
| Secret `lakekeeper-oidc`                                                                                                                                                 | `client-id`, `client-secret` — **the** centrally-known Casdoor client: the worker authenticates to Lakekeeper with it, the Console shows it to the customer                                                                                          | [casdoor seed Job](./charts/casdoor/templates/seed-job.yaml) (seeds the matching Casdoor application) |
| Secret `duckflight-auth-secrets`                                                                                                                                         | `auth-tokens`, `auth-users`, `basic-username`, `basic-password` (token surfaced in the Console)                                                                                                                                                      | [duckflight/values.yaml](./charts/duckflight/values.yaml)                                             |
...
- [casdoor seed](./charts/casdoor/templates/seed-job.yaml) — `casdoor-init-data`,
...
- [gitea seed](./charts/gitea/templates/seed-job.yaml) — `gitea-secrets`
- [rill seed](./charts/rill/templates/seed-job.yaml) — `rill-oidc`, `rill-git`
  (Gitea access token for the snapshot sidecar), `rill-snapshot-auth`,
  `platform-git`
- [dlt seed](./charts/dlt/templates/seed-job.yaml) — `dlt-git`, `dlt-age` (the box
  ...
  `transformations-git`
- [workspace-api seed](./charts/workspace-api/templates/seed-job.yaml) —
...
[dlt-worker](./charts/dlt/templates/statefulset.yaml), whose dlt filesystem
destination writes Iceberg data files straight to the bucket, the
[iceberg-maintenance CronJob](./charts/iceberg-maintenance/templates/cronjob.yaml)
(direct data-file IO during compaction), and the
[backup CronJob](./charts/backup/templates/cronjob.yaml) (uploads nightly dumps to
...
[postgres/ensure-databases-job.yaml](./charts/postgres/ensure-databases-job.yaml)
...
the last category with no day-2 path: [box-secrets/](./charts/box-secrets/) fetches
...
Worse than fixed, until [bootstrap-retire/](./charts/bootstrap-retire/): what
...
| New **databases**                                                                          | [postgres/ensure-databases-job.yaml](./charts/postgres/ensure-databases-job.yaml), converges every sync                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| New **box-local credentials**                                                              | idempotent seed Job: create-Secret-if-absent (+ Casdoor app via management API where needed). Pattern: [gitea/templates/seed-job.yaml](./charts/gitea/templates/seed-job.yaml) + [seed-rbac.yaml](./charts/gitea/templates/seed-rbac.yaml) (negative sync waves: RBAC → seed → consumers)                                                                                                                                                                                                                                                                                   |
| **Casdoor identity seed** (org, applications, admin user, JWT cert)                        | rendered on the box by [casdoor/templates/seed-job.yaml](./charts/casdoor/templates/seed-job.yaml) from [casdoor/values.yaml](./charts/casdoor/values.yaml) `initDataApplications`; `initDataNewOnly = true` makes Casdoor apply it create-if-absent, and the JWT signing key is generated by Casdoor itself (never in Terraform state). Editing the seeded shape = fleet rollout                                                                                                                                                                                           |
| **ArgoCD version**                                                                         | [argocd/](./charts/argocd/) adopts the bootstrap `HelmChart` CR; bump `argocdChartVersion` in [charts/root/values.yaml](./charts/root/values.yaml). Reverted at every k3s start until the box's cloud-init manifest is retired ([bootstrap-retire/](./charts/bootstrap-retire/)) — `version` is the one thing the `HelmChartConfig` cannot outrank                                                                                                                                                                                                                                        |
| **ArgoCD values** (QoS requests, probes, params)                                           | [argocd/templates/helmchartconfig.yaml](./charts/argocd/templates/helmchartconfig.yaml) — merged over the frozen cloud-init `HelmChart`, so this one converges on legacy boxes without waiting for retirement                                                                                                                                                                                                                                                                                                                                                        |
| **Root app spec** (syncPolicy, new fleet-wide parameters)                                  | self-managed [charts/root/templates/root-app.yaml](./charts/root/templates/root-app.yaml)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Ring branch** of a box, **repo deploy token**, values of the **contract Secrets**        | edit in-cluster (ops break-glass) — sticks only once [bootstrap-retire/](./charts/bootstrap-retire/) has written the sentinel for that box, otherwise the next reboot restores the provisioning-day value                                                                                                                                                                                                                                                                                                                                                            |
| **k3s version**                                                                            | [system-upgrade/](./charts/system-upgrade/): bump `k3sVersion` in [charts/root/values.yaml](./charts/root/values.yaml) (the cloud-init `var.k3s_version` install pin only matters at first boot — keep the two moving together)                                                                                                                                                                                                                                                                                                                                                    |
...
[rill seed](./charts/rill/templates/seed-job.yaml), which also mints the sidecar's
...
([casdoor/templates/middleware-gitea-logout.yaml](./charts/casdoor/templates/middleware-gitea-logout.yaml)).
...
(private; created by the [dlt seed](./charts/dlt/templates/seed-job.yaml), which
...
the box by [dlt-seed](./charts/dlt/templates/seed-job.yaml) step 4 (agekey init
...
[casdoor-seed](./charts/casdoor/templates/seed-job.yaml)). The FairTier API trusts
the box's issuer (`auth.customer-<slug>.<baseDomain>`) and binds the tenant
from the issuer host.
Rollback lever in [dlt/values.yaml](./charts/dlt/values.yaml):
...
Grafana Alloy ([observability/](./charts/observability/), chart `grafana/alloy`
1.12.1) pushes **outbound-only** to
...
- **Metrics** → central Prometheus `remote_write`, external labels
  ...
  **PostgreSQL** (see below). A keep-list in
  the [River config](./charts/observability/config/templates/configmap.yaml)
...
[observability/config/values.yaml](./charts/observability/config/values.yaml), the
...
[box-secrets/](./charts/box-secrets/) as Secret `betterstack-telemetry` in
...
([postgres/statefulset.yaml](./charts/postgres/statefulset.yaml), public image on
...
1. **The ArgoCD values moved to a
   [`HelmChartConfig`](./charts/argocd/templates/helmchartconfig.yaml).** The k3s
...
2. **[bootstrap-retire/](./charts/bootstrap-retire/)** writes a k3s `.skip` sentinel
...
Nightly CronJob `box-backup` ([backup/](./charts/backup/), 03:30 UTC — after
...
Job](./charts/workspace-api/templates/seed-job.yaml)). A restore onto a *fresh*
...
Retention 7 days ([values.yaml](./charts/backup/values.yaml)), matching the
...
- ⚠️ **Rill's vended-credentials read path** — verified on `v0.87.2`; the
  pin is now `v0.87.8` and this check is **owed on the canary**, because it
  is the one assertion here that is about the image rather than the config.
  It is the one deliberate delta from
  ...
  but through the DuckDB/iceberg-extension build *bundled in the Rill
  image*, which is older than DuckFlight's. A patch on the 0.87 line does
  not move that build, which is why this is a re-check and not a rewrite.
  Check:
...
- ✅ **Casdoor's `built-in` bootstrap admin is rotated at seed** (2026-07-17).
  ...
  [casdoor-seed](./charts/casdoor/templates/seed-job.yaml) step 5 replaces the
...
- 🟡 ArgoCD adopting the bootstrap `HelmChart` CR ([argocd/](./charts/argocd/)): the
...
- ✅ The self-managed root app ([charts/root/templates/root-app.yaml](./charts/root/templates/root-app.yaml)):
...
- ✅ **Backup CronJob** ([backup/](./charts/backup/)) — manual run validated on the
...
- [PUBLIC.md](./PUBLIC.md) — what publishing this tree means, and where the
  images come from
- [charts/root/values.yaml](./charts/root/values.yaml) — the fleet-wide version pins and
